...
//!
//! 664 of the artefact's 696 `not(…)` assertions are represented,
//! as 1548 element and 21 attribute rows — more rows than assertions,
//! because an alternation like
//! `(cac:InvoiceLine|cac:CreditNoteLine)/x` is one rule and two paths.
//!
//! [`UNEXTRACTED`] = 32 are not represented: their test is
...
//! public so a test reports "664 of 696 checked" rather than
...
/// Every `UBL-*` `not(…)` assertion in the artefact, checkable or not.
///
/// One per **assertion**. The tables above hold 1569 rows, which is more,
