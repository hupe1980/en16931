...
**The EN 16931 validator as a command.** UBL, CII and ZUGFeRD / Factur-X in,
a verdict out — text for a person, JSON for a pipeline, SVRL for every other
...
static binary that answers the same question, from the same artefacts, and tells
you which **business term** is wrong rather than which XPath.
...
it is why XRechnung is not a *conformant CIUS* under §4.4.2. The
[`en16931` README](../en16931#️-severity-is-the-authoritys-not-ours) has the
evidence.

The two runs differ in one word, and that word is the point. Every finding leads
with its severity, and the header counts them —
...
EN 16931 says the document means — not a transliteration of its elements. That
is why a UBL invoice whose BT-21 is embedded in the note text as `#AAI#…` comes
...
cannot carry — BT-11 on a credit note is the known case — goes to **stderr**, so
...
The embedded payload of a ZUGFeRD / Factur-X PDF, **verbatim**. Whoever
diagnoses a rejected invoice needs the bytes the counterparty sent, not a
reconstruction of them. Disagreements between the PDF's XMP metadata and the
payload are reported on stderr.
...
What the file is, without a verdict on it: syntax, declared BT-24, which rule
set that resolves to *here*, the parties, the totals, and anything the reader
could not map. The first command to run on a document you were sent.
...
counterparty months later. [`en16931-formats`](../en16931-formats) explains
