//! A **test-only** UBL reader, for running the authorities' conformance suites.
//!
//! This crate ships no XML and will not — see the layering in `README.md`. But
//! CEN, KoSIT and OpenPeppol all publish their conformance suites as UBL
//! instances with per-rule expectations, and those suites are the only way to
//! check that a rule *means* what the Schematron means rather than merely
//! carrying its id. Reading them needs a reader; the reader belongs here, in
//! `tests/`, where it is not part of the crate's API, its dependency graph, or
//! its wasm build.
//!
//! The production reader lives in `xrechnung`. This one is deliberately not
//! shared: a dependency from `en16931` to a format crate — even a dev one —
...
//! A conformance run that quietly ignored `cbc:TaxExclusiveAmount` would report
//! a green suite and prove nothing, which is the same failure mode
//! `tests/corpus.rs` exists to prevent. The conformance test asserts the
//! unmapped set against an explicit allowlist.
                    ...
                    if let Some(r) = kid(c, "InvoiceDocumentReference") {
                        inv.preceding_invoices.push(PrecedingInvoice {
                            reference: text(r, "ID")
                                ...
                                .unwrap_or_else(|| DocumentReference::new("")),
