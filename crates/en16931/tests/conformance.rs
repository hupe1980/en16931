...
//! **must not**. ~1 130 assertions in total. Crucially each is scoped to one rule,
...
/// The coverage floors, measured against CEN `validation-1.3.16` and KoSIT's
/// `master` on 2026-07-31.
///
/// # Why floors and not exact counts
///
/// Two of the three suites come from repositories pinned to a **moving
/// branch** — KoSIT's `xrechnung-schematron` and OpenPeppol's
/// `peppol-bis-invoice-3` track `master`, because neither publishes releases on
/// a cadence this crate can pin to. So the totals move under us, and they have:
/// the documentation claimed 1 131 assertions, 1 017 agreements and 382
/// mutations, and the artefacts now yield 1 129, 1 013 and 381.
///
/// Equality would turn every upstream commit into a red build. A floor keeps
/// the claim that matters — *coverage does not shrink* — while letting upstream
/// grow. The other half of the claim, 100 % agreement, is asserted exactly.
const MIN_CEN_ASSERTIONS: usize = 1_000;
/// See [`MIN_CEN_ASSERTIONS`]. 381 mutations run of 449 runnable.
const MIN_KOSIT_MUTATIONS: usize = 350;
/// See [`MIN_CEN_ASSERTIONS`]. 58 published example invoices.
...
fn cen_conformance_suite() {
    ...
    //
    // 100 % agreement over three assertions is 100 %, and would be worthless.
    // The percentage is only meaningful next to how much was run, and nothing
    // asserted that — so a fetch that landed a truncated corpus, or a directory
    // rename upstream, would have passed with a smaller denominator and no
    // sign of it.
    //
    // A floor rather than equality because `eInvoicing-EN16931` is pinned to a
    // tag but the suite grows across tags; upstream *adding* cases must not
    // fail the build. Losing them must. Raise the floor when a bump raises the
    // count, and never lower it without saying why in the commit.
    assert!(
